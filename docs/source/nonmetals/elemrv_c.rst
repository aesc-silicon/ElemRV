...
ElemRV-C is the second-tier Nonmetal platform. It builds on Hydrogen's foundation
with a focus on execution speed: the CPU gains an instruction cache, hardware
multiply/divide (M extension), compressed instructions (C extension), and a
single-cycle barrel shifter. The peripheral set and pinmux are otherwise
...
.. list-table::
   ...
   * - PWM0
     - ``0x3000``
     - 4 kB
     - PWM controller, 1 channel - see :ref:`hardware-peripherals-pwm`
   * - UART0
     - ``0x4000``
     - 4 kB
...
The ECPIX5 target maps ElemRV-C pins to the following board resources:
...
The SG13CMOS5L target places IO pads around the chip perimeter:
